...
public class CodeA3ML extends JPanel {
	...
	public static TransformGroup create_Fan() {
		...
		fanTG.addChild(create_Base("ML's Assignment 3"));  // create and attach "Base" to "FanStand"
		return fanTG;
	}
	...
	public static BranchGroup create_Scene() {
		...
		//sceneTG.addChild(CommonsML.rotate_Behavior(7500, sceneTG));
	...
	public CodeA3ML(BranchGroup sceneBG) {
		GraphicsConfiguration config = SimpleUniverse.getPreferredConfiguration();
		Canvas3D canvas = new Canvas3D(config);
		
		SimpleUniverse su = new SimpleUniverse(canvas);    // create a SimpleUniverse
		CommonsML.define_Viewer(su, new Point3d(0.25d, 0.25d, 10.0d));   // set the viewer's location
	...
	static class Key extends KeyAdapter {
		...
		public void keyTyped( KeyEvent e) {
			...
			if( keyChar == 'z' || keyChar == 'Z') {
				...
				if ( f )	object3D[2].get_Alpha().pause(); 	
				else	object3D[2].get_Alpha().resume(); 	
				f = (f) ? false : true;
	...
	public static void main(String[] args) {
		frame = new JFrame("ML's Assignment");                   // NOTE: change XY to student's initials
		frame.getContentPane().add(new CodeA3ML(create_Scene()));  // start the program
