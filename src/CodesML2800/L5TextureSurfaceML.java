...
public class L5TextureSurfaceML extends GroupObjects {
	...
	public static TransformGroup round_Table(int n) {      // define scaling factors
		...
		String[] side_name = {"Top", "Side" , "Bottom"};   // add bottom surface 
		SharedGroup shared3D = new SharedGroup( );	
		for (int i = 0; i < 3; i++) 		               // share the disk's two sides
			shared3D.addChild(L5TextureSurfaceML.ring_Shape(side_name[i], n)); 
		...
		for (int i = 0; i < 5; i++) {
			...
			linkTG.addChild(L5TextureSurfaceML.link_OneDisk(post[i], links[i], scl[i]));
	...
	private static TriangleStripArray ring_Side(String shape_key, int n) {
		...
		Point3f c_pts[] = L2StarML.circle_Points(0, r, n);                
	...
	public static Appearance set_Appearance(String s) {
		...
		app.setTexture(L5TextureSurfaceML.texture_Appearance("Image" + s));
		...
		if ( s == "Bottom") {	
		    ...
		    rotation.rotX(Math.PI);  //Rotate Texture 180 degree (flip vertically)
		    
		    moveRight.setTranslation(new Vector3d(-0.25, 0.0, 0.0));  // Move texture right
	...
	public L5TextureSurfaceML(String s) {
