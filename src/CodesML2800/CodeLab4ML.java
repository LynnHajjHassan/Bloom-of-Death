...
public class CodeLab4ML extends JPanel implements ActionListener {

	private static final long serialVersionUID = 1L;
	private static JFrame frame;
	
	private static String frame_name = "ML's Lab #4";
	...
	public static float[] gapsArray = { 0, 0.25f , 0.5f, 0.75f } ;  // gaps
	...
	private static BranchGroup create_Scene() {
		...
		Color3f[] side_color = {CommonsML.Orange, CommonsML.Green , CommonsML.Purple};  // I added a a color for the new surface here 
		                                                   // create and attach the (stationary) side surface
		sceneTG.addChild(L4TransformML.ring_Side(1, side_color[1]));
		...
		sideTG.addChild(L4TransformML.ring_Side(0, side_color[0]));
		...
		sideTG2.addChild(L4TransformML.ring_Side(2, side_color[2]));  // 2 indicating that we need to generate the bottom surface 
		...
		CommonsML.control_Rotation(r_tag);                 // make 'alterableBG' rotating by default
	...
	public CodeLab4ML(BranchGroup scene) {
	...
	public static void main(String[] args) {
		...
		frame.getContentPane().add(new CodeLab4ML(create_Scene()));
	...
	public void actionPerformed(ActionEvent e) {
		...
		case OBJECT_NAME:
			...
			gapTF2.setTranslation(new Vector3f(0f, 0f, (float)( -(gapsArray[gaps_index]) * gap_tag))); 
			sideTG2.setTransform(gapTF2);  
			
			if ( gaps_index == 3) {  gaps_index = 0 ;}
			else { 
				 gaps_index++ ; 
			}
