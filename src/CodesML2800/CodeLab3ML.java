...
public class CodeLab3ML extends JPanel implements ActionListener {

	private static final long serialVersionUID = 1L;
	private static JFrame frame;
	
	private static String frame_name = "ML's Lab #3";
	...
	private static BranchGroup create_Scene() {
		...
		groupObject = new GroupObjects(L3DiskML.ring_Side(2.0f));
	...
	public CodeLab3ML(BranchGroup scene) {
		...
		CommonsML.define_Viewer(su, new Point3d(1.35, -0.35, 10.0)); 		// set the viewer's location
	...
	public static void main(String[] args) {
		...
		frame.getContentPane().add(new CodeLab3ML(create_Scene()));
	...
	public void actionPerformed(ActionEvent e) {
		...
		case "Pause/Rotate":
			...
			CommonsML.control_Rotation(r_tag);             // alter between rotation and pause
			return;
		case OBJECT_NAME:
	        // Cycle through the disk sizes
			float currentSize = L3DiskML.getRadius();  // Get the current disk size
	        if (currentSize == 2.0f) {  // Current size is large (2.0)
	            groupObject = new GroupObjects(L3DiskML.ring_Side(1.5f));  // Change to medium size (1.5)
	        } else if (currentSize == 1.5f) {  // Current size is medium (1.5)
	            groupObject = new GroupObjects(L3DiskML.ring_Side(1.0f));  // Change to small size (1.0)
	        } else if (currentSize == 1.0f) {  // Current size is small (1.0)
	            groupObject = new GroupObjects(L3DiskML.ring_Side(0.5f));  // Change to tiny size (0.5)
	        } else {  // Current size is tiny (0.5)
	            groupObject = new GroupObjects(L3DiskML.ring_Side(2.0f));  // Restore to original size (2.0)
	        }
	        break;
		default:
			return;
		...
		// Update the frame title with the new disk size
	    float newSize = L3DiskML.getRadius();
	    frame.setTitle(frame_name + ": Disk size = " + newSize);
