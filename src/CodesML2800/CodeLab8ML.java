...
/* Copyright material by MLuan@uwindsor.ca,
...
public class CodeLab8ML extends JPanel implements ActionListener {

	private static final long serialVersionUID = 1L;
	private static JFrame frame;
	
	private static String frame_name = "ML's Lab #8";
	...
	public static TransformGroup move_Cube(Alpha alpha) {
		...
		PositionInterpolator positionInterpol = new PositionInterpolator(alpha, moveTG, axisPosition, -4f, 4f);		
		...
		CollisionDetectShape collision = new CollisionDetectShape(colorCube);
		collision.setSchedulingBounds(CommonsML.twenty_BS);        // detect column's collision
		moveTG.addChild(collision);
	...
	private static BranchGroup create_Scene() {
		...
		CodeLab7ML.rotate_Side(sceneBG, sceneTG, alpha);   // make the two flipping (side) surfaces 
	...
	public CodeLab8ML(BranchGroup scene) {
	...
	public static void main(String[] args) {               // NOTE: copyright material
		...
		frame.getContentPane().add(new CodeLab8ML(create_Scene()));
