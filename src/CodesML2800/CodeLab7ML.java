...
public class CodeLab7ML extends JPanel implements ActionListener {

	private static final long serialVersionUID = 1L;
	private static JFrame frame;
	
	private static String frame_name = "ML's Lab #7";
	...
	public static void rotate_Side(BranchGroup snBG, TransformGroup snTG, Alpha[] aph) {
		...
		Transform3D slide, slide2, plate, rotate_axis, rotate2, plate2;
		TransformGroup slideTG, slide2TG, plateTG, plate2TG, hingeTG, hinge2TG;
		...
		plateTG.addChild(L5TextureSurfaceML.ring_Shape(side_name[0], 60));
		
		plate2 = new Transform3D();                         // shift the circular surface's far end to rotational origin
		plate2.setTranslation(new Vector3f(2.0f, 0, 0.1f));
		plate2TG = new TransformGroup(plate2);               // need 'plateTG' to position circular surface for rotation
		plate2TG.addChild(L5TextureSurfaceML.ring_Shape(side_name[2], 60));
		...
		rotat = new RotationInterpolator(aph[1], hinge2TG, rotate2, 0, (float) (Math.PI / 2.0)); 
		...
		snTG.addChild(slide2TG);
		snTG.addChild(L5TextureSurfaceML.ring_Shape(side_name[1], 60));
	}
	...
	public CodeLab7ML(BranchGroup scene) {
	...
	public static void main(String[] args) {               // NOTE: copyright material
		...
		frame.getContentPane().add(new CodeLab7ML(create_Scene()));
