...
public class Room {
	...
	// Room dimensions
    public static double roomWidth = 50.0;  
    public static double roomLength = 100.0; 
    protected static double roomHeight = 35.0;
    public static TransformGroup extraWallTG; // So main can access it
	
	// Method used to connect the room's walls.
	// Input: nothing, Output: TransformGroup roomTG 
	public static TransformGroup createEmptyRoom() {
		 ...
		 TransformGroup roomTG = new TransformGroup();
		   ...
		   // Appearance ceilingAppearance = CommonsLH.set_Appearance(CommonsLH.Grey); // White ceiling
		    ...
		    ExtraWall extraWall= new ExtraWall(); 
            TransformGroup extraWallTG = extraWall.position_Object();
            roomTG.addChild(extraWallTG);

            // Expose the extra wall's TG to the outside
            Room.extraWallTG = extraWallTG;
...
class ExtraWall extends BODObjects{
	public ExtraWall() {
		scale = 31d;                                        // use to scale up/down original size
		post = new Vector3f(27f, 4.5f, -67f);                  // use to move object for positioning
		transform_Object("extraWall");                          // set transformation to 'objTG' and load object file
	...
	protected void create_Appearance() {
	    ...
	    mtl_clr[0] = new Color3f(0.3f, 0.3f, 0.3f);  // Ambient color (light gray)
	    mtl_clr[1] = new Color3f(0.3f, 0.3f, 0.3f);  // Diffuse color (same as ambient for uniform lighting)
	    mtl_clr[2] = new Color3f(0.0f, 0.0f, 0.0f);  // Specular color (black to remove shininess)
	    mtl_clr[3] = new Color3f(0.0f, 0.0f, 0.0f);  // Emissive color (black, no glow)

	    setShininess(0);  // Remove shininess
