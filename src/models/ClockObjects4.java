...
class ClockHouse extends ClockObjects4 {
	public ClockHouse() {
		...
		obj_Appearance("textures/clockmaincolor.jpg");                                  // set appearance after converting object node to Shape3D
	}
...
class ClockBase extends ClockObjects4 {
	public ClockBase() {
		...
		obj_Appearance("textures/one.jpg");                                  // set appearance after converting object node to Shape3D
	}
...
class ClockCenter extends ClockObjects4 {
	...
	public ClockCenter() {
		...
		obj_Appearance("textures/CC2.jpg");
...
class ClockTop extends ClockObjects4 {
	public ClockTop() {
		...
		obj_Appearance("textures/clockTop.jpg");                                 // set appearance after converting object node to Shape3D
	}
...
class ClockSide1 extends ClockObjects4 {
	public ClockSide1() {
		...
		obj_Appearance("textures/dong3.jpg");                                  // set appearance after converting object node to Shape3D
	}
...
class ClockTree1 extends ClockObjects4 {
	public ClockTree1() {
		...
		obj_Appearance("textures/greentree.jpg");                                  // set appearance after converting object node to Shape3D
	}
...
class TreeBase1 extends ClockObjects4 {
	public TreeBase1() {
		...
		obj_Appearance("textures/treebase.jpg");                                  // set appearance after converting object node to Shape3D
	}
...
class HourHand extends ClockObjects4 {
    public HourHand() {
        ...
        obj_Appearance("textures/hands.png");
    }
    ...
    public void updateRotation(float angle) {
        Transform3D baseRotation = new Transform3D();
        baseRotation.rotX(-Math.PI / 2); // Original base rotation to lie flat

        Transform3D rotation = new Transform3D();
        rotation.rotZ(angle); // Manual angle adjustment

        baseRotation.mul(rotation); // Combine both transforms
        objRG.setTransform(baseRotation);
    }
...
class MinuteHand extends ClockObjects4 {
    public MinuteHand() {
        ...
        obj_Appearance("textures/hands.png"); // Set appearance
    }
...
class ClockDong extends ClockObjects4 {
    public ClockDong() {
       ...
       // post = new Vector3f(0.1f, -0.5f, 0.5f); // Position the pendulum below the ClockHouse
        ...
        obj_Appearance("textures/dong3.jpg"); // Set appearance
    }

    public TransformGroup position_Object() {
        float dongLength = 1.2f; // Adjust to match actual model height
        float extraFix = 0.1f;   // Tweak this if the pivot still feels off

        objRG = new TransformGroup();
        objRG.setCapability(TransformGroup.ALLOW_TRANSFORM_WRITE);

        Transform3D rot = new Transform3D();
        rot.rotX(Math.PI / 2); // Align vertically
        objRG.setTransform(rot);

        // Adjust model downward so pivot hits top of stick
        Transform3D pivotFix = new Transform3D();
        pivotFix.setTranslation(new Vector3f(0f, -(dongLength / 2f + extraFix), 0f));
        TransformGroup offsetTG = new TransformGroup(pivotFix);
        offsetTG.addChild(objBG);

        objRG.addChild(offsetTG);

        // Move entire structure back to visual spot
        Transform3D restore = new Transform3D();
        restore.setTranslation(new Vector3f(-0.2f, post.y + dongLength / 2f + extraFix, post.z));
        objTG = new TransformGroup(restore);

        objTG.addChild(objRG);
        applySwingMotion(2000);
        return objTG;
    }
