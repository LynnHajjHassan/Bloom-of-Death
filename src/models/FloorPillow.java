...
public class FloorPillow {
	
	private static BODObjects TLObjects;
	
	public static TransformGroup create_FloorPillow() {
		TransformGroup Pillow = new TransformGroup();
		
		//handle positions object
		TLObjects = new PillowObj();
		Pillow = TLObjects.position_Object();
...
class PillowObj extends BODObjects {
	public PillowObj() {
		scale = 5d;                                        	// use to scale up/down original size
		post = new Vector3f(24f, -16.5f, 42f);                   // use to move object for positioning
		transform_Object("FloorPillow");                      	// set transformation to 'objTG' and load object file
	...
	protected void create_Appearance() {
		...
		mtl_clr[2] = new Color3f(0.0f, 0.0f, 0.0f) ;
	  ...
	  app.setTexture(textured_App("fabric"));     		
	}
