...
public class TableLamp {
	...
	public  static TransformGroup create_TableLamp() {
...
class TLBase extends BODObjects{
	public TLBase() {
		scale = 1.6d;                                        	// use to scale up/down original size
		post = new Vector3f(42f, -8f, 40f);                   // use to move object for positioning
...
class TLBulb extends BODObjects {
  ...
  protected void create_Appearance() {
      // Defining colors, and setting them on the shape       
      mtl_clr[0] = new Color3f(0.2f, 0.2f, 0.2f);
      mtl_clr[1] = new Color3f(0.3f, 0.3f, 0.35f);        // diffuse color
  ...
  private void create_Light() {
      ...
      bulbLight.setColor(new Color3f(1.0f, 1.0f, 1.0f)); 			
      bulbLight.setPosition(new Point3f(post.x, post.y, post.z));  // Position the light at the bulb's location
      bulbLight.setInfluencingBounds(new BoundingSphere(new Point3d(0.0, 0.0, 0.0), 0.1));  // Set the light's influence bounds
...
class TLShade extends BODObjects {
	    ...
	    private void create_Light() {
	        ...
	        bulbLight = new PointLight();
	        bulbLight.setColor(new Color3f(1.0f, 1.0f, 0.9f));  // Warm white light
