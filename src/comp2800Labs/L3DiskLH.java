...
public class L3DiskLH extends GroupObjects {

	private final static int MAX_PTS = 60; // number of points to approximate the circle
	
	public static Shape3D ring_Side(float r) {  // create a 3d disk object with radius r 
		int k, n = MAX_PTS;
		
		int s_num = 3;                      // make three of the approximated (curve) surfaces
		...
		Point3f c_pts1[] = L2StarLH.circle_Points(-0.1f, r, MAX_PTS);
		Point3f c_pts2[] = L2StarLH.circle_Points(0.1f, r, MAX_PTS);
		
		Point3f ctr_pt = new Point3f(0f, 0f, -0.1f);  // the centre
		Point3f ctr_pt2 = new Point3f(0f, 0f, 0.1f);  // the centre
		...
		for (int i = 0; i <= n; i++) {
			...
			v_clrs[i * 2] = v_clrs[i * 2 + 1] = CommonsLH.Orange;
			
			v_cdnts[v_num + i * 2] = c_pts2[k];            // set for outside (vertical, curve) surface
			v_cdnts[v_num + i * 2 + 1] = c_pts1[k];
			v_clrs[v_num + i * 2] = v_clrs[v_num + i * 2 + 1] = CommonsLH.Green;
			
			v_cdnts[(v_num * 2) + i * 2] = ctr_pt2;    // Back surface center (flipped)
			v_cdnts[(v_num * 2) + i * 2 + 1] = c_pts2[k];  // Back surface edge (flipped)
			v_clrs[(v_num * 2) + i * 2] = v_clrs[(v_num * 2) + i * 2 + 1] = CommonsLH.Purple;
