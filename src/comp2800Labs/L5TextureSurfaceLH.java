...
public class L5TextureSurfaceLH extends GroupObjects {
	...
	public static TransformGroup round_Table(int n) {      // define scaling factors
		Vector3d[] scl = {new Vector3d(0.25, 0.25, 2), new Vector3d(1, 1, 1) , new Vector3d(0.25, 0.25, 2), 
				new Vector3d(0.25, 0.25, 2),new Vector3d(0.25, 0.25, 2)};    
		Vector3f[] post = {new Vector3f(1.4f, 0, -0.2f), new Vector3f(0, 0, 0.1f) ,new Vector3f(0f, 1.4f ,-0.2f),
				new Vector3f(-1.4f, 0 ,-0.2f), new Vector3f(0f, -1.4f ,-0.2f)};
                                                           // define shifting distances
		// this part is used to create the table surfaces 
		String[] side_name = {"Top", "Side" , "Bottom"};   // added the bottom surface 
		SharedGroup shared3D = new SharedGroup( );	
		for (int i = 0; i < 3; i++) 		               // share the disk's two sides
			shared3D.addChild(L5TextureSurfaceLH.ring_Shape(side_name[i], n)); 
		shared3D.compile();                                // optimize the group to be shared
		
		// this part is used to create tables legs 
		TransformGroup linkTG = new TransformGroup();   // this will holds all the table legs 
		Link[] links = new Link[5];  
		for (int i = 0; i < 5; i++) {
			links[i] = new Link(shared3D);                 // link two disks for table's top and leg 
			linkTG.addChild(L5TextureSurfaceLH.link_OneDisk(post[i], links[i], scl[i]));
	...
	private static TriangleStripArray ring_Side(String shape_key, int n) {
		...
		Point3f c_pts[] = L2StarLH.circle_Points(0, r, n);                
		Point3f ctr_pt = new Point3f(0f, 0f, 0.1f);
		Point3f ctr_pt2 = new Point3f(0f, 0f, -0.1f);   // create another centre point for the bottom part 
		Point3f p1, p2;
		...
		for (int i = 0; i <= n; i++) {
			...
			}else if ( shape_key == "Bottom") {		//NEW: added for the bottom surface 
					p1 = new Point3f(c_pts[k].x, c_pts[k].y, -0.1f);  // NEW: changed 0.1 to -0.1
					p2 = ctr_pt2;
					nml = new Vector3f(0f, 0f,  -1f);  // NEW: modified 1 to -1 for z 
			}
	...
	public static Appearance set_Appearance(String s) {
		...
		app.setTexture(L5TextureSurfaceLH.texture_Appearance("Image" + s));
		...
		if ( s == "Bottom") {	
			float scl = 0.24f;  // Adjusted scale for the bottom texture
		    Vector3d scale = new Vector3d(scl, scl, scl); 

		    //Create Transform3D objects
		    Transform3D transMap = new Transform3D();  // to scale it 
		    Transform3D rotation = new Transform3D(); // to rotate it
		    Transform3D moveRight = new Transform3D();  // to move it to the right 

		    //Apply transformations
		    transMap.setScale(scale);// apply scaling
		    
		    //rotation.rotX(Math.PI);  //Rotate Texture 180 degree (flip vertically), this was for Lab5
		    rotation.rotY(Math.PI);  // NEW LAB6: to flip the image 

		    
		    //moveRight.setTranslation(new Vector3d(-0.25, 0.0, 0.0));  // Move texture right, for Lab5 
		    moveRight.setTranslation(new Vector3d(0, 0.0, 0.0));  // Lab6 NEW 


		    //Combine Transformations
		    transMap.mul(rotation);  // Apply rotation to scaling
		    transMap.mul(moveRight);  // Apply translation to the combined transformations

		    //Apply final transformation to the texture
	...
	public L5TextureSurfaceLH(String s) {
