...
public class L4TransformLH extends GroupObjects {
	...
	public static Shape3D ring_Side(int shape_key, Color3f clr) {
		...
		Point3f c_pts[] = L2StarLH.circle_Points(0, r, MAX_PTS);                
		Point3f ctr_pt = new Point3f(0f, 0f, 0.1f);
		Point3f ctr_pt2 = new Point3f(0f, 0f, -0.1f);    // added by Lynn: this is the centre of the bottom circular surface
		Point3f p1, p2;
		...
		for (int i = 0; i <= n; i++) {
			...
			}else if ( shape_key > 1) {                  // if we need to generate the bottom surface 
				p1 = new Point3f(c_pts[k].x, c_pts[k].y, -0.1f);
				p2 = ctr_pt2;
				nml = new Vector3f(0f, 0f, -1f);		    // normal vector that is responsible for object lighting 
	...
	public L4TransformLH(int key) {
		super(ring_Side(key, CommonsLH.Green));
	}
}