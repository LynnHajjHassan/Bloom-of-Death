...
class L2StarLH extends GroupObjects {
	...
	private static LineArray one_Star(int num, Color3f clr, float r) {
		...
		for (int i = 0; i <  num; i++) {                   // define lines for the start
			...
			lineArr.setCoordinate(i * 2 + 1, c_pts[(i + 1) % num]);   // change this to + 1 
			lineArr.setColor(i * 2, clr);    
			lineArr.setColor(i * 2 + 1, CommonsLH.White);  // set color for end points
		}
	...
	public static Shape3D line_Shape(float r) {
		int n = 60;          // change thise to 40                               // create an n-sided start with 'r' radius
		LineArray starGeometry = one_Star(n, CommonsLH.White, r); 

		return new Shape3D(starGeometry);
	}
	

	public L2StarLH(float r) {
		super(line_Shape(r));
	}
